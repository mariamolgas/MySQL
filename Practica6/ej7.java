...
query.constrain(Directores.class);
query.descend("nacionalidad").constrain("Estadounidense").not();
ObjectSet<Directores> d = query.execute();

Iterator<Directores> itp = d.iterator();
    while (itp.hasNext()){
      System.out.println(itp.next().toString());
}

final Query querya=db.query();
querya.constrain(Actores.class);
querya.descend("nacionalidad").constrain("Estadounidense").not();
ObjectSet<Actores> a = querya.execute();

Iterator<Actores> ita = a.iterator();
    while (ita.hasNext()){
