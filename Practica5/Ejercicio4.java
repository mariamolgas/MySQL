...
ra=stmt.executeUpdate("Drop TABLE if exists Directores");
 ...
 ra=stmt.executeUpdate("CREATE TABLE if not exists Directores(ID_director INT,	nombre VARCHAR(100) NOT NULL, PRIMARY KEY (ID_director),unicidad VARCHAR(9) NOT NULL,	UNIQUE (unicidad),edad INT NOT NULL,CONSTRAINT chk CHECK (edad>0 AND edad<120),	nacionalidad VARCHAR(100) DEFAULT 'Estadounidense', creacion TIMESTAMP NOT NULL DEFAULT NOW());");
